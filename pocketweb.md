...
**程序同时只能运行一个实例。** 用命名管道 `PocketWeb-SingleInstance` 实现：

- 第二个实例启动时先探测管道，连得上说明已有实例在运行 → 发 `show` 通知对方激活主窗口，自己**立即退出**（根本不会创建窗口）；
- 发消息前先调 `AllowSetForegroundWindow(ASFW_ANY)` 授权对方置前 —— 否则 Windows 的前台焦点保护会让已有窗口只能闪一下任务栏、起不来；
- 收到对方的 `ok` 应答后本实例才退出，保证这次授权在对方激活窗口期间仍然有效；
- 已有实例若正**最小化或隐藏在托盘**，`showWindow()` 会把它恢复并置前 —— 即「最小化后再启动程序就激活它」。

管道名带 `PocketWeb-` 前缀，与 DSH-Environment 完全独立：两个程序可以同时运行，各自只允许一个实例。

> 之所以坚持单实例，还因为多开会共用同一份 WebEngine profile 目录与配置文件（Chromium 不支持同一 profile 目录被多进程共用；`webapplets.json` 也会并发覆盖写）。
...
- **已裁剪掉的部分**：DSH 对话窗口、CMD 终端窗口、DSH 源码管理、导航栏的树形菜单与底部按钮列。

---

## 10. 本文档说明

本文档由代码静态梳理而成，**未经过编译与运行验证**；若与代码不符，以代码为准。
